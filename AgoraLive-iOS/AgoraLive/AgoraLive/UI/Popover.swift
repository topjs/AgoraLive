...
protocol PopoverDelegate: NSObjectProtocol {
    func popoverDidDismiss(_ popover: Popover)
}
...
open class Popover: UIView {
    ...
    override open func layoutSubviews() {
        ...
        let y = arrowSize.height + CGFloat(distance)
        let width = self.bounds.width - CGFloat(distance) * 2
        let height = self.bounds.height - arrowSize.height - CGFloat(distance * 2)
    ...
    override open func draw(_ rect: CGRect) {
        ...
        case .down, .auto:
            arrow.move(to: CGPoint(x: arrowPoint.x, y: 1))
            arrow.addLine(
                to: CGPoint(
                    x: arrowPoint.x + self.arrowSize.width * 0.5 - 1,
                    y: self.arrowSize.height + 1
                )
            )
            arrow.addLine(to: CGPoint(x: arrowPoint.x -  self.arrowSize.width * 0.5 + 1, y: self.arrowSize.height + 1))
            arrow.addLine(to: CGPoint(x: arrowPoint.x, y: 1))
