...
private extension AGESingleCamera {
    ...
    func sessionConfiguration(position: Position, preview: AGECameraPreview?, olds: OldItems) throws {
        ...
        if #available(iOS 10.0, *) {
            guard let tCamera = AVCaptureDevice.default(.builtInWideAngleCamera,
                                                       ...
                                                       position: position) else {
                                                        throw AGEError(type: .fail("no \(position.description) camera"))
            }
            camera = tCamera
        } else {
        ...
        guard session.noumenon.canAddInput(newCameraDeviceInput) else {
                throw AGEError(type: .fail("session can not add \(position.description) camera input"))
        }
        ...
        guard session.noumenon.canAddOutput(newVideoDataOutput) else {
            throw AGEError(type: .fail("no \(position.description) camera output"))
        }
        ...
        if #available(iOS 13.0, *) {
            guard let tVideoPort = newCameraDeviceInput.ports(for: .video,
                                                             ...
                                                             sourceDevicePosition: camera.position).first else {
                                                                throw AGEError(type: .fail("no \(position.description) camera input's video port"))
            }
        ...
        if let preview = preview {
            ...
            guard session.noumenon.canAddConnection(newPreviewConnection!) else {
                throw AGEError(type: .fail("no a connection to the \(position.description) camera video preview layer"))
            }
