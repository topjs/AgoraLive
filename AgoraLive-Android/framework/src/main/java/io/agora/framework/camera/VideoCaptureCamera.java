...
package io.agora.framework.camera;
...
public class VideoCaptureCamera
    ...
    public boolean allocate(final int width, final int height, final int frameRate, final int facing) {
        ...
        parameters.setPreviewFormat(mCaptureFormat.getPixelFormat());
        try {
        ...
        mExpectedFrameSize = mCaptureFormat.getWidth() * mCaptureFormat.getHeight()
                * ImageFormat.getBitsPerPixel(mCaptureFormat.getPixelFormat()) / 8;
    ...
    public void startCaptureMaybeAsync(boolean needsPreview) {
        ...
        synchronized (mCameraStateLock) {
            if (mCameraState == CameraState.STOPPING) {
                ...
                if (mTexId == -1) mTexId = GlUtil.createTextureObject(GLES11Ext.GL_TEXTURE_EXTERNAL_OES);
